...
export function UpdatePanel({ checking, onCheckUpdates, onOpenReleaseNotes }: Props) {
  return (
    <section className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <article className="overflow-hidden rounded-[22px] border border-white/[0.09] bg-[#111113] shadow-card">
        <div className="flex flex-col gap-5 border-b border-white/[0.07] p-6 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-6 text-white/46">
              Alle LunaSuite Apps werden direkt über den Launcher aktualisiert. So bleiben die einzelnen Apps schlank, schnell und sauber.
            </p>
          </div>
          <LauncherButton
            variant="primary"
        ...
        <div className="grid gap-px bg-white/[0.06] sm:grid-cols-3">
          <UpdateStat icon={CheckCircle2} label="LunaMail v1.0.1" value="Update verfügbar" />
          <UpdateStat icon={ShieldCheck} label="Sicherheitsstatus" value="Update vorbereitet" />
          <UpdateStat icon={Clock3} label="Letzte Prüfung" value="vor 2 Stunden" />
        </div>
        ...
        >
          <span className="flex items-center gap-3">
            <History size={17} />
            Update-Verlauf und Release Notes anzeigen
          </span>
      ...
      <article className="rounded-[22px] border border-white/[0.09] bg-[#111113] p-6 shadow-card">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-semibold">Sicher installiert</h2>
            <p className="mt-1 text-sm leading-6 text-white/42">Downloads werden vor der Installation geprüft.</p>
          </div>
        </div>

        <div className="mt-6 space-y-1">
          {updateHistory.slice(0, 3).map((item) => (
...
function UpdateStat({ icon: Icon, label, value }: { icon: typeof RefreshCw; label: string; value: string }) {
  return (
    <div className="bg-[#111113] px-6 py-5">
      <Icon size={17} className="mb-4 text-white/55" />
      <p className="text-sm font-medium">{label}</p>
      <p className="mt-1 text-xs text-white/38">{value}</p>
    </div>
  );
}
